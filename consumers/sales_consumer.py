...
def animate(i):
    ...
    axes[0, 0].set_title("Sales Heatmap: Product vs Payment", backgroundcolor='blue')
    ...
    axes[0, 1].set_title("Sales Count by Product Category", backgroundcolor='purple')
    axes[0, 1].tick_params(axis='x', rotation=45)
    
    axes[0, 2].pie(df_bar["sales_count"], labels=df_bar["product_category"], autopct='%1.1f%%', colors=sns.color_palette("pastel"))
    axes[0, 2].set_title("Sales Proportion by Product Category", backgroundcolor='green')
    
    df["timestamp"] = pd.to_datetime(df["timestamp"])
    df_line = df.set_index("timestamp").resample("1T").size()
    df_line.plot(ax=axes[1, 0], marker='o', linestyle='-')
    axes[1, 0].set_title("Sales Trend Over Time", backgroundcolor='red')
    
    sns.histplot(df["price"], bins=20, kde=True, ax=axes[1, 1])
    axes[1, 1].set_title("Sales Price Distribution", backgroundcolor='blue')
    
    sns.boxplot(x="product_category", y="price", data=df, palette="Set3", ax=axes[1, 2])
    axes[1, 2].set_title("Price Variation by Product Category", backgroundcolor='red')
...
if __name__ == "__main__":
    ani = FuncAnimation(plt.gcf(), animate, interval=1500000)
    plt.show()